...
> ** ML 연구를 통해 다양한 도메인·데이터 문제 해결하는데 관심이 있고, 이제는 MLOps로 확장 중인 컴퓨터공학 학부생**
> 금융·화장품/의료·유전체·그래프·분산 빅데이터까지 서로 다른 데이터 유형을 다뤄 왔고,
> 그 경험을 앞으로 **MLOps** 방향으로 확장하고자 합니다.
...
컴퓨터공학을 전공하며 **표현공간에서의 분리 학습**, **멀티모달 학습**, **응용 머신러닝** 에 관심을 가지고 연구를 해왔고, 
더 나아가 **MLOps**에 관심을 두고 연구·프로젝트를 진행하고자 합니다.

저는 **하나의 태스크가 아니라, 서로 다른 도메인과 데이터 유형을 폭넓게 경험**해왔습니다. 정형 금융 거래 데이터부터 얼굴·피부 이미지, 유전체 시퀀스, 거래 그래프, 대규모 분산 빅데이터까지 다뤄 보며, 다양한 형태의 데이터와 모델 흐름을 공부할 수 있었습니다.
...
| 프로젝트 | 분야 | 한 줄 요약 | 상세 |
|---|---|---|---|
| 표현 공간 분리 사기 탐지 | 표현학습 · 금융 보안 | 정상/사기 거래를 표현 공간에서 분리해 탐지 | [보기](projects/fraud-representation-separation/README.md) |
| 3D 얼굴 골격 화장 에이전트 | 멀티모달 · 생성 | 3D 얼굴 구조 분석 + 멀티모달 생성 기반 맞춤 화장 추천 | [보기](projects/3d-face-cosmetic-agent/README.md) |
| 멀티모달 피부 분석·추천 | 멀티모달 학습 | 피부 상태 멀티모달 분석 기반 화장품 추천 | [보기](projects/multimodal-skin-analysis/README.md) |
| 자금세탁 위험도 스코어링 | 그래프 · 설명가능 ML | 암호화폐 거래 그래프 기반 chain-level 위험도 스코어링 | [보기](projects/graph-money-laundering-risk/README.md) |
| AI 생성 이미지 탐지 (PEFT) | 컴퓨터 비전 · PEFT | LN-tuning vs. LoRA 비교로 AI 생성 이미지 탐지 | [보기](projects/ai-image-detection-peft/README.md) |
| 유전체 언어 모델 (AMR) | NLP · 유전체 | 항생제 내성(AMR) 예측용 소형 유전체 언어 모델 | [보기](projects/genomic-language-model-amr/README.md) |
| SARS-CoV-2 K-mer 분산처리 | 빅데이터 · 바이오 | 유전체 K-mer 빈도 분석의 분산 처리 최적화 | [보기](projects/sars-cov-2-kmer-analysis/README.md) |
