...
> 이 역량은 **제조 AI·스마트팩토리**(설비 이상 탐지/PHM · 머신비전 불량 검사 · 대용량 데이터 처리 · MLOps)로 자연스럽게 전이됩니다.

<p align="center">
  <a href="#-핵심-역량">핵심 역량</a> ·
  <a href="#-연구-방향성과-여정">연구 여정</a> ·
  <a href="#-스마트팩토리제조-ai-연결성">제조 AI 연결성</a> ·
...
특히 이 역량들은 **스마트팩토리·제조 AI**의 핵심 과제와 맞닿아 있습니다. 정상/이상 패턴 분리와 불균형 데이터 처리는 **설비 이상 탐지(PHM)** 로, 이미지 판별은 **머신비전 기반 불량 검사** 로, 이미지·정형 결합은 **센서·영상 이종 데이터 분석** 으로, 대용량 분산 처리는 **공장 센서·로그 데이터 파이프라인** 으로 이어집니다. 아직 제조 현장 데이터로 직접 수행한 프로젝트는 아니지만, 습득한 역량이 제조 AI 문제로 어떻게 **전이되는지**를 아래 [제조 AI 연결성](#-스마트팩토리제조-ai-연결성) 섹션에 정리했습니다.

## 🧭 핵심 역량

- **머신러닝 / 딥러닝** — 표현학습, 분류, 시퀀스 모델링, 컴퓨터 비전
- **이상 탐지 · 불균형 데이터** — 정상/이상 패턴 분리 *(→ 설비 이상 탐지·PHM)*
- **컴퓨터 비전** — 이미지 판별, PEFT(LoRA/LN-tuning) *(→ 머신비전 불량 검사)*
- **멀티모달** — 이미지·정형·생성 신호 결합 *(→ 센서·영상 이종 데이터 검사)*
- **그래프 · 설명 가능 ML** — 관계 데이터 모델링, 위험도 스코어링, 해석 가능성 *(→ 공정·설비·공급망 위험 분석)*
- **데이터 · 분산 처리** — 대규모 데이터 전처리, 분산 처리 최적화 *(→ 공장 센서·로그 파이프라인)*
- **MLOps (관심·확장 중)** — 재현 가능한 파이프라인, 확장 가능한 데이터/모델 워크플로 *(→ 라인 운영 배포)*
...
> 아래 표는 **실제 제조 현장 데이터로 수행한 프로젝트가 아니라**, 각 프로젝트에서 습득한 역량이 제조 AI 문제로 어떻게 **전이될 수 있는지**를 정리한 것입니다.

| 프로젝트 | 제조 AI 응용 (전이 가능성) | 전이되는 핵심 역량 |
|---|---|---|
| 표현 공간 분리 사기 탐지 | **설비 이상 탐지 (PHM)** | 정상/이상 패턴 분리, 불균형 데이터 처리 |
| AI 생성 이미지 탐지 (PEFT) | **머신비전 기반 표면·불량 검사** | 이미지 판별, 효율적 미세조정 |
| 멀티모달 피부 분석·추천 | **센서+영상 이종 데이터 검사** | 이미지·정형 멀티모달 분석 |
| 자금세탁 위험도 스코어링 | **공정·설비·공급망 관계 데이터 위험 분석** | 관계형(그래프) 모델링, 설명 가능성 |
| SARS-CoV-2 K-mer 분산처리 | **대규모 센서·로그 데이터 처리** | 대용량 데이터·분산 처리 |
| (전반) MLOps 관심 | **모델을 라인 운영 파이프라인으로** | 재현성, 확장성, 운영 배포 |
| 3D 얼굴 골격 분석 | **부품 3D 형상·치수 검사** *(참고)* | 3D 구조 분석 |

---

## ⭐ 대표 프로젝트

제조 AI 관련성과 연구 깊이를 함께 보여주는 세 가지입니다.

### 1. 표현 공간 분리를 활용한 금융 사기 탐지 &nbsp;·&nbsp; `이상 탐지` `표현학습` `KCC 2026`

> 정상/사기 거래를 **잠재 표현 공간에서 분리**하여 탐지 성능과 해석 가능성을 높이는 연구.

- **핵심 아이디어** — 원본 특징 공간에서 겹치는 사기 신호를, 학습된 표현 공간에서 더 잘 구분되도록 분리
- **🏭 제조 연결** — 정상/이상 분리 + 불균형 데이터 처리 → **설비 이상 탐지(PHM)** 로 전이 가능
...
### 2. AI 생성 이미지 탐지를 위한 PEFT (LN-tuning vs. LoRA) &nbsp;·&nbsp; `머신비전` `PEFT`

> 사전학습 비전 모델을 **파라미터 효율적으로 미세조정(PEFT)** 하여 이미지의 진위/판별 성능을 비교한 연구.

- **핵심 아이디어** — LN-tuning과 LoRA를 비교해, 적은 학습 파라미터로 효율적인 이미지 판별 달성
- **🏭 제조 연결** — 이미지 판별 + 효율적 미세조정 → **머신비전 기반 표면·불량 검사** 로 전이 가능 (엣지·소량 데이터 환경에 유리)
- **주요 결과** — *[Add result: LN-tuning vs. LoRA 정확도/효율 비교]* · *[Add figure]*
- **기술 스택** — Python, PyTorch *[확인]* · Hugging Face `transformers` / `peft` *[확인]*
- 📄 [상세 보기 »](projects/ai-image-detection-peft/README.md)

### 3. SARS-CoV-2 유전체 K-mer 빈도 분산처리 &nbsp;·&nbsp; `빅데이터·분산처리` `과제`

> 대규모 유전체 데이터의 **K-mer 빈도 분석을 분산 처리로 최적화** — 모델링을 넘어 **확장성·데이터 파이프라인**으로 관심이 넓어진 출발점.

- **핵심 아이디어** — 단일 노드로는 확장되지 않는 K-mer 카운팅을 분산 처리로 최적화
- **🏭 제조 연결** — 대용량·분산 처리 경험 → **공장 센서·로그 데이터 파이프라인** 및 MLOps로 전이 가능
...
- **관심 분야** — AI 연구 · 표현학습 · 멀티모달 학습 · MLOps · 응용 머신러닝 · 제조 AI/스마트팩토리

> 연구 협업, 대학원 연구 기회, 제조 AI·스마트팩토리, ML 엔지니어링 관련 문의를 환영합니다.
